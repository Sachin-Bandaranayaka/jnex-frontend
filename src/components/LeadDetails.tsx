        ...
        <div className="bg-white rounded-lg shadow p-6 space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div className="space-y-2">
                    <h2 className="text-2xl font-bold">Lead #{lead.leadNo}</h2>
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <span className={`px-2 py-1 rounded ${lead.score >= 50 ? 'bg-green-100' : 'bg-yellow-100'}`}>
                            {lead.score}
                        </span>
                    </div>
                </div>
                    ...
                    {lead.tasks?.map((task: Task) => (
                        <div key={task.id} className="border rounded p-3">
                            ...
                            <div className="mt-2 flex items-center space-x-2">
                                <span className={`px-2 py-1 rounded text-sm ${task.priority === 'high' ? 'bg-red-100' :
                                    task.priority === 'medium' ? 'bg-yellow-100' :
                                        'bg-green-100'
                                    }`}>
                                    {task.priority}
                                </span>
                                <span className={`px-2 py-1 rounded text-sm ${task.status === 'completed' ? 'bg-green-100' :
                ...
                <div className="space-y-2">
                    {lead.interactionHistory?.map((interaction, index) => (
                        <div key={index} className="border-l-4 border-blue-500 pl-3 py-2">
                            <div className="text-sm text-gray-500">
                                {new Date(interaction.timestamp).toLocaleString()} - {interaction.user}
                            </div>
                            <div className="mt-1">
                                <span className="font-medium">{interaction.type}:</span> {interaction.notes}
                            </div>
                        </div>
                    ))}
                </div>
